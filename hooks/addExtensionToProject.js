// run when iOS platform or plugin is added
...
// get peferences added for plugin from config.xml
...
// add values from config into plist
...
module.exports = function (context) {
  ...
  fs.readdir(iosFolder, function (err, data) {
    ...
    var run = function () {
      ...
      if (pbxGroupKey) {
        log(
          'Successfully created empty PbxGroup for folder: ' +
            widgetName +
            ' with alias: SevenCooksWidgetExtension',
          'info',
        )
      }
      ...
      var frameworkFile1 = pbxProject.addFramework('SwiftUI.framework', {
        target: target.uuid,
      })
      var frameworkFile2 = pbxProject.addFramework('WidgetKit.framework', {
        target: target.uuid,
